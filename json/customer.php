...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $year = date("Y");
    $mon = date("M");
    //customer,custLog,custPass,driveLog,driver,driverPass,finaLog,finaPass,inveLog,
    //invePass,staff,supplier,suppLog,suppPass
    ...
    if (mysqli_num_rows($query) > 0) {
        ...
        $select = "SELECT contact FROM customer WHERE contact='$contact'";
        $query = mysqli_query($con, $select);
        if (mysqli_num_rows($query) > 0) {
            $response["success"] = 0;
            ...
            $values = $hits[0];
            $reg = $values . $year;
            $sql = "INSERT INTO customer(reg,fname,lname,email,password,country,city,contact,address)

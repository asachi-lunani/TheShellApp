...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (mysqli_num_rows($query) > 0) {
        ...
        $select = "SELECT contact FROM staff WHERE contact='$contact'";
        $query = mysqli_query($con, $select);
        if (mysqli_num_rows($query) > 0) {
            $response["success"] = 0;
            ...
            $values = $hits[0];
            $reg = $values . $year;
            $sql = "INSERT INTO staff(serial,fname,lname,email,password,role,country,city,contact,address)
    VALUES('$reg','$fname','$lname','$email','$password','$role','$country','$city','$contact','$address')";
